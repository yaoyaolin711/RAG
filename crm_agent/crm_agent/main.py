...
融合 **CRM 达人回复 Agent** 与 **RAG 知识库微信客服**，对外提供一组标准 REST API。
...
| 接口 | 说明 |
|------|------|
| `POST /api/wechat/chat` | 与 `/api/v1/chat` 相同，均走 UnifiedReplyAgent |
| `POST /api/talents/{id}/simulate` | 达人消息模拟，同样走 UnifiedReplyAgent，响应字段略有差异 |
...
app.include_router(wechat_router, prefix="/api/wechat", tags=["微信 RAG（兼容）"])
app.include_router(talents_router, prefix="/api/talents", tags=["达人回复（兼容）"])
